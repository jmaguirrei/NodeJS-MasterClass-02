...
module.exports = {
  ...
  get(data, callback) {
    ...
    const id = helpers.checkAndReturn(data.queryStringObject, 'id', { length: 20 });
    if (id) {
  ...
  put(data, callback) {
    ...
    const id = helpers.checkAndReturn(data.payload, 'id', { length: 20 });
    const extend = helpers.checkAndReturn(data.payload, 'extend', { asBoolean: true });
